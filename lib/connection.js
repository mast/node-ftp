...
FTP.prototype.connect = function(options) {
  ...
  this.options.socketTimeout = options.socketTimeout || 1000;
  ...
  var noopreq = {
        ...
        cb: function() {
          ...
          clearTimeout(self._noopcbtimer);
  ...
  function donoop() {
    if (!self._socket || !self._socket.writable)
      ...
      self._noopcbtimer = setTimeout(() => {
          self.emit('noop.timeout')
      }, self.options.socketTimeout);
...
FTP.prototype._reset = function() {
  ...
  clearTimeout(this._keepalive);
  this._keepalive = undefined;
  clearTimeout(this._noopcbtimer);
  this._noopcbtimer = undefined;
  this._queue = [];
  this._ending = false;
  this._parser = undefined;
